...
class PrintLogger(QObject):
    ...
    def write(self, text): # write the input text in the following way
        if self.n == 1:
...
def meanDataXYZ(matrix_data, resolution=2, data_less=10):
    X, Y, Z, Zstd, Zcounts, h, k1, l = [], [], [], [], [], [], [], []

    scaled_resolution = int(resolution * 4)
    matrix = np.full((40 * scaled_resolution, 38 * scaled_resolution), fill_value=None, dtype=object)

    print('Sort Data')

    for i in range(len(matrix_data[:, 9])):
        tmp = matrix_data[i:i + 1, :]
        row_index, col_index = int(matrix_data[i, 9] * resolution), int(matrix_data[i, 10] * resolution)
        if matrix[row_index, col_index] is None:
            matrix[row_index, col_index] = tmp * resolution
        else:
            matrix[row_index, col_index] = np.concatenate((matrix[row_index, col_index], tmp * resolution))
...
class connectButton(qt5_interface.Ui_MainWindow, QMainWindow):
    def __init__(self):
        super().__init__()
...
#%%
    ...
    def evaluate_load_data_update(self):
        if self.thread_evaluate_loadData.is_alive():
            ...
            self.logNewSubline(self.logfile_eval_path, f'Calculated mean height of data: {mean}\n')
    ...
    def leveling_data(self):
        if type(self.evaluate.mergeDataSet) == int:
            ...
            self.logNewLine(self.logfile_eval_path, 'Sputter/Ersoion leveling was applied')
            self.logNewSubline(self.logfile_eval_path, f'Mean height before leveling: {self.evaluateMeanEroisionlineEdit.text()}')
            self.logNewSubline(self.logfile_eval_path, f'Reference height given: {self.evaluateRefereceLevelLineEdit.text()}')
            self.logNewSubline(self.logfile_eval_path, f'Mean height after leveling: {self.lineEditAfterLeveling.text()}\n')
    ...
    def levelingOxidData(self):
        if type(self.evaluate.mergeDataSet)== int:
            ...
            self.logNewSubline(self.logfile_eval_path, f'Height of oxide layer: {self.evaluateOxideHeightLineEdit.text()} micrometers')
            self.logNewSubline(self.logfile_eval_path, f'Compensate grow in depth: {self.evaluateDepthGrowlineEdit.text()}')
            ...
            self.evaluateMeanEroisionlineEdit.setText( str(np.round(np.mean(self.evaluate.mergeDataSet[:,2]),4)))
            print('Leveling button is disabled. Only one time the depth growth should be multiply to the data')
    ...
    def browseLevelingDataFilters(self):
        if type(self.evaluate.mergeDataSet)== int:
            ...
            if self.evaluateNoiseFilterCheckBox.isChecked():
                self.logNewSubline(self.logfile_eval_path, f'Noise filter size: {self.evaluateNoiseFilterSize.text()}')
            if self.evaluateNoiseFilterCheckBox.isChecked():
                self.logNewSubline(self.logfile_eval_path, f'Noise filter size: {self.evaluateNoiseFilterSize.text()}')
            self.evaluateApplyFilterButton.setEnabled(False)
            self.evaluateProgressBar.setMaximum(0)
