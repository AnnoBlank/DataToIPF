...
class PrintLogger(QObject):
    ...
    def write(self, text): # write the input text in the following way
        if self.n == 1:
            ...
            self.newText.emit(str(datetime.now(pytz.utc).time())[:-7] + ':  '+  str(text))
            self.n=0
        else:
            self.n=1
...
def meanDataXYZ(mat, resolution=2, data_less=10):
    X, Y, Z, Zstd, Zcounts, h, k1, l = [], [], [], [], [], [], [], []

    resolution_tmp = int(resolution * 4)
    matrix = np.full((40 * resolution_tmp, 38 * resolution_tmp), fill_value=None, dtype=object)

    print('Sort Data')

    for i in range(len(mat[:, 9])):
        tmp = mat[i:i + 1, :]
        row_index, col_index = int(mat[i, 9] * resolution), int(mat[i, 10] * resolution)
    ...
    for k in range(38 * resolution_tmp):
        for o in range(35 * resolution_tmp):
            tmp = matrix[k, o]
            if tmp is None:
                continue
    ...
    print('Cut off filter used: ', less_data)
    print('IPF data point: ', data_count)
...
class connectButton(qt5_oberflaeche.Ui_MainWindow, QMainWindow):
    ...
    def tabMergeCLSM(self): 
        ...
        self.mergeviewCLSM.clicked.connect(self.browse_merge_view_CLSM)
    ...
    def load_clsm_data_thread(self, mode = 'norm'):   
        ...
        if (self.loadCLSM1line.text() == '' or self.loadCLSM2line.text() == ''):
            ...
            if len(self.mergedata.confocal_data) == 2 or self.data_merge_clsm_single == 1:
                ...
                if mode == 'norm':
                    self.thread.finished.connect(self.load_clsm1_2_data_thread_finished) 
                elif mode == 'auto':
                    self.thread.finished.connect(self.load_auto_clsm_data_thread_finished)
                elif mode == 'load':
                    self.thread.finished.connect(self.load_auto_clsm_data_thread_finished_from_file)
                else:
                    print('Unknown mode!')
    ...
    def  load_clsm_data_thread_finished(self):
        ...
        if not self.loadCLSM1line.text() and not self.loadCLSM2line.text():
            self.select_points_status = 0
        elif self.select_points_status == 1:
            self.select_points_status = 0
            self.select_points()
