...
class parent_merge(QObject):
    ...
    def render_confocal_image(
        ...
        return ls.shade(
            M, cmap=cmap, vert_exag=10, blend_mode="hsv", vmin=min_scale, vmax=max_scale
        )

    def pattern_matching_auto(
        self,
        depth=25,
        leveling=1,
        levelplotrange=1,
        ...
        try:
            ...
            image_data_1 = (
                (image_data_1 - lowerBoundCal) / (-lowerBoundCal + upperBoundCal) * 256
            )
            ...
            image_data_2 = (
                (image_data_2 - lowerBoundCal) / (-lowerBoundCal + upperBoundCal) * 256
            )
            ...
            P1 = np.array(
                [kp1[j].pt for j in np.array([i.queryIdx for i in matches])]
            ).astype(int)
            P2 = np.array(
                [kp2[j].pt for j in np.array([i.trainIdx for i in matches])]
            ).astype(int)
    ...
    def load_confocal_data_diff_plt(
        self,
        leveling=1,
        levelplotrange=1,
        followup_selection=True,
    ):
        try:
            ...
            if followup_selection:
                ...
                textstr1 = "\n".join(
                    (
                        r"Please select the points at indentical features. First select point in left CLSM data than",
    ...
    def confocal_diff_from_file(
        self,
        file_name="tmp/000_selected_points_difference_microscopy.txt",
        leveling=1,
        levelplotrange=1,
    ):
        try:
    ...
    def affine_transformation(
        ...
        if np.max(P1) < np.max(P2):
            m, n, dataouttmp = self.sub_tform(
                P1, P2, self.confocal_data_1, self.confocal_data_2
            )

        else:
            m, n, dataouttmp = self.sub_tform(
                P2, P1, self.confocal_data_2, self.confocal_data_1
            )
    ...
    def render_confocal_data(self):
        ...
        try:
            ...
            anzahlColums = np.loadtxt(
                file_name, delimiter=",", dtype="str", skiprows=25 + len(anzahlrows) - 1
            )
            confocal_data = np.loadtxt(
                file_name,
                delimiter=",",
                skiprows=25,
                usecols=list(
                    np.linspace(
                        1, len(anzahlColums) - 2, len(anzahlColums) - 1, dtype=int
                    )
                ),
            )
    ...
    def load_confocal_data_diff(self):
        for CLSM_set in range(2):
            ...
            try:
                anzahlrows = np.loadtxt(
                    file_name, delimiter=",", skiprows=25, usecols=[5]
                )
                anzahlColums = np.loadtxt(
                    file_name,
                    delimiter=",",
                    dtype="str",
                    skiprows=25 + len(anzahlrows) - 1,
                )
                confocal_data = np.loadtxt(
                    file_name,
                    delimiter=",",
...
class mergeThread(parent_merge):
    ...
    def load_confocal_data(self):
        try:
            ...
            print(
                "Reading file: %s, please wait... this can take several minutes..."
                % file_name
            )
            anzahlrows = np.loadtxt(file_name, delimiter=",", skiprows=25, usecols=[5])
            anzahlColums = np.loadtxt(
    ...
    def load_EBSD_data(self):
        try:
            ...
            XEBSDPicture = -1 * np.ones(
                num,
            )
            YEBSDPicture = -1 * np.ones(
                num,
            )
            ...
            colorz = (
                colorEBSDPicture[:, 0] + colorEBSDPicture[:, 1] + colorEBSDPicture[:, 2]
            )
            ...
            HKL[:, 0] = np.sin((Orientation[:, 1]) * np.pi / 180) * np.sin(
                Orientation[:, 2] * np.pi / 180
            )
            HKL[:, 1] = np.sin((Orientation[:, 1]) * np.pi / 180) * np.cos(
                Orientation[:, 2] * np.pi / 180
            )
...
class mergedata(parent_merge):
    ...
    def load_confocal_data2(
        ...
        data = np.loadtxt(
            ...
            skiprows=25,
            usecols=list(
                np.linspace(1, len(anzahlColums) - 2, len(anzahlColums) - 1, dtype=int)
            ),
        )
        data = data[:, 1:]
        if rot > 0:
    ...
    def load_confocal_data1_rot_flip(self):
        if hasattr(self, "confocal_data_1"):
            ...
            if rot > 0:
                ...
                self.confocal_data = self.confocal_data_1
                self.confocal_image = self.render_confocal_image(
                    data=self.confocal_data
                )
    ...
    def save_diff_points(
        ...
        np.savetxt(
            "tmp/000_selected_points_difference_microscopy.txt",
    ...
    def remove_confocal_data(
        ...
        for number in range(0, number_of_removals):
            ...
            self.confocal_data[
                int(P[0][0]) : int(P[1][0]), int(P[0][1]) : int(P[1][1])
            ] = -1
            plt.close("all")
        self.confocal_image = self.render_confocal_image(self.confocal_data)
    ...
    def view_confocal_data(
        ...
        if self.P.shape != (2,):
            plt.plot(
                self.P[:, 0], self.P[:, 1], "or", linewidth=3, markersize=12, alpha=0.5
            )
    ...
    def view_EBSD_data(self):
        ...
        ax_ebsd.imshow(
            ...
            aspect=np.max(self.Y) / np.max(self.X),
        )
    ...
    def view_pic_data(self):
        ...
        plt.imshow(
            ...
            cmap=plt.cm.gray,
            aspect=np.max(self.Ytiff) / np.max(self.Xtiff),
        )
        plt.colorbar()
    ...
    def calibrate_confocal_and_EBSD_data_image(self):
        ...
        if self.Pc.shape != (2,):
            ax1.plot(
                self.Pc[:, 0], self.Pc[:, 1], "ob", linewidth=3, markersize=12, alpha=1
            )
        ...
        if self.P.shape != (2,):
            ax2.plot(
                self.P[:, 0], self.P[:, 1], "ob", linewidth=3, markersize=14, alpha=1
            )
    ...
    def calibrate_confocal_and_EBSD_data(
        self,
        P_keep=0,
        Pc_keep=0,
    ):
        ...
        if type(P_keep) != int and type(Pc_keep) != int:
            ax2.plot(
                P_keep[:, 0], P_keep[:, 1], "ob", linewidth=3, markersize=6, alpha=0.7
            )
            for i in range(len(P_keep)):
                ax2.text(P_keep[i, 0], P_keep[i, 1], "        selected", fontsize=12)

            ax1.plot(
                Pc_keep[:, 0], Pc_keep[:, 1], "ob", linewidth=3, markersize=6, alpha=0.7
            )
        ...
        textstr1 = "\n".join(
            (
                r"Please select the points at indentical features. First select point in EBSD data than",
        ...
        self.save_selected_points_merge = (
            "tmp/" + self.nowstr + "_selected_points_merge.txt"
        )
    ...
    def calculate_superposition(self):
        ...
        f = interpolate.interp2d(
            x, y.T, M, kind="linear", bounds_error=False, fill_value=-1
        )
        ...
        for k in range(0, self.HKL_result.shape[0]):
            self.XY_result[k, :] = ipfzXY.poleA2(
                self.HKL_result[k, 0], self.HKL_result[k, 1], self.HKL_result[k, 2]
            )
    ...
    def delteDataThroughImage(
        ...
        if CLSM_Data == 1:
            ...
            x = np.linspace(
                1, self.imageForDeleteData.shape[1], self.imageForDeleteData.shape[1]
            )
            y = np.linspace(
                1, self.imageForDeleteData.shape[0], self.imageForDeleteData.shape[0]
            )
            f = interpolate.interp2d(
                y, x, M, kind="linear", bounds_error=False, fill_value=-1
            )
            pic_contrast_del = np.zeros((len(self.new_coordinates), 1))
            for i in range(len(self.new_coordinates)):
                pic_contrast_del[i] = f(
                    self.new_coordinates[i, 0], self.new_coordinates[i, 1]
                )
    ...
    def delte_Data_Through_Image_Plot(self):
        ...
        axcalc3.scatter(
            self.X_result,
            self.Y_result,
            c=self.Heights_result.reshape(len(self.Heights_result)),
            marker="o",
            s=10,
            cmap=plt.cm.gray,
            alpha=0.1,
    ...
    def data_zero_level(
        ...
        textstr1 = "\n".join(
            (
                r"Please select a reference height with the left mouse button",
        ...
        self.ax_clsm.text(
            ...
            0.05,
            textstr1,
            transform=self.ax_clsm.transAxes,
            fontsize=12,
            verticalalignment="top",
            bbox=props,
        )
    ...
    def save_confocal_data(
        ...
        np.savetxt(
            ...
            self.dataclsm,
            fmt="%.5e",
            header=" X Y Height PHI PSI2 PSI1 H K L X Y   ___datamerge_1_32",
        )
        return 1
    ...
    def save_tiff_data(
        ...
        np.savetxt(
            Samplename + "_merge_Contrast_data_1_31.dat",
            tosave,
            fmt="%.5e",
            header=" X Y Contrast PHI PSI2 PSI1 H K L IPFz_X_Coordinate IPFz_Y_Coordinate",
        )
        return 1
