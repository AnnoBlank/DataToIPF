...
def rotation(
    ...
    R = np.array(
        [
            [
                cos_phi1 * cos_phi2 - cos_phi * sin_phi1 * sin_phi2,
                -cos_phi * cos_phi2 * sin_phi1 - cos_phi1 * sin_phi2,
                sin_phi * sin_phi1,
            ],
            [
                cos_phi2 * sin_phi1 + cos_phi * cos_phi1 * sin_phi2,
                cos_phi * cos_phi1 * cos_phi2 - sin_phi1 * sin_phi2,
                -cos_phi1 * sin_phi,
            ],
...
def cristalStruc(
    ...
    V = (
        ...
        * np.sqrt(
            ...
            - (np.cos(bet)) ** 2
            - (np.cos(gam)) ** 2
            + 2 * b * c * np.cos(alp) * np.cos(bet) * np.cos(gam)
        )
    )
    D = np.array(
        [
